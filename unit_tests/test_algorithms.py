...
def test_tf_idf_multiple_call() -> None:
    ...
    expected2 = [
        {'doc_id': 1, 'text': 'hello', 'tf_idf': 0.34657359027997264},
        {'doc_id': 1, 'text': 'world', 'tf_idf': 0.34657359027997264},

        {'doc_id': 2, 'text': 'little', 'tf_idf': 0.6931471805599453}
    ]

    result2 = graph.run(texts=lambda: iter(rows2))

    assert expected2 == sorted(result2, key=itemgetter('doc_id', 'text'))


def test_pmi_multiple_call() -> None:
    graph = algorithms.pmi_graph('texts', doc_column='doc_id',
    ...
    rows2 = [
        {'doc_id': 1, 'text': 'hello, LITt WORLD'},
        {'doc_id': 2, 'text': 'LITt'},
        {'doc_id': 3, 'text': 'LITtw LITtw LITtw'},
        {'doc_id': 4, 'text': 'LITtw? hello LITtw world'},
        {'doc_id': 5, 'text': 'HELLO HELLO! WORLD...'},
        {'doc_id': 6, 'text': 'world? world... world!!! WORLD!!! HELLO!!! HELLO!!!!!!!'}
    ]

    expected2 = [  # Mind the order !!!
        {'doc_id': 3, 'text': 'littw', 'pmi': approx(0.9555, 0.001)},
        {'doc_id': 4, 'text': 'littw', 'pmi': approx(0.9555, 0.001)},
        {'doc_id': 5, 'text': 'hello', 'pmi': approx(1.1786, 0.001)},
        {'doc_id': 6, 'text': 'world', 'pmi': approx(0.7731, 0.001)},
        {'doc_id': 6, 'text': 'hello', 'pmi': approx(0.0800, 0.001)},
    ]
...
def test_yandex_maps_multiple_call() -> None:
